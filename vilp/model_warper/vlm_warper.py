...
class VLM_Warper(BasePredictor):
    def __init__(
        self,
        model_type="llava",
        ...
        if model_type == "llava":
            from ivlm.model_warper.llava_warper import Predictor

            self.predictor = Predictor()
            self.predictor.setup(checkpoint_path, model_name)
        elif model_type == "cambrian":
            from ivlm.model_warper.cambrian_warper import Cambrian_Predictor

            self.predictor = Cambrian_Predictor()
            self.predictor.setup(checkpoint_path, model_name, conv_mode)
        elif model_type == "mgm":
            # mini-gemini
            from ivlm.model_warper.mgm_warper import MGM_Predictor
