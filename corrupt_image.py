...
parser.add_argument(
    "--output_file",
    type=str,
    default="semantic_corrupted_40k_v2.json",
)
parser.add_argument(
    "--use_random_blur",
    action="store_true",
...
parser.add_argument(
    ...
    choices=["llava-v1.5-7b", "llava-v1.5-13b"],
)
...
if args.corruption_type == "semantic":
    json_colelct = glob.glob(
        os.path.join(
            args.file_folder,
            f"*/*groundingdino*/single_img_QA_DPO_{args.model_name}.json",
        )
    )
elif args.corruption_type == "classic":
    json_colelct = glob.glob(
        os.path.join(args.file_folder, f"*/*/single_img_QA_DPO_{args.model_name}.json")
    )
else:
    raise ValueError(f"Corruption type {args.corruption_type} not supported")
...
while num < NUM_LIMIT and json_colelct:
    ...
    for item in data:
        ...
        if args.corruption_type == "semantic":
            ...
            bad_image = np.array(Image.open(bad_image_path).convert("L"))
            good_image = np.array(Image.open(image_path).convert("L"))
            ...
            bad_image = Image.open(image_path)
