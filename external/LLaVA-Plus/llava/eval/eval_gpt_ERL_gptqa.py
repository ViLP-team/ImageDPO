...
if __name__ == "__main__":
    ...
    for item in generated_instructions:
        ...
        cur_key = item["question_id"] + str(item["question_index"])
        ...
        content = f"""
        We are conducting an evaluation of an AI assistant's ability to provide answer for a piar of images. The AI assistant was given a original image and a modified image and tasked with predicting "{question}".
...
text_output = text_output.replace(".jsonl", "_score.txt")
with open(text_output, "w") as f:
    f.write(f"ERL final score:: {np.mean(score_results)*20}\n")
