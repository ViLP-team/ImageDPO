...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="/nfs/turbo/justincj-turbo/tiangel/improvingVLM/packages/LLaVA-Plus/playground/eval_data/two_img_dataset/ActivityDataset/gpt_qsa_activity.jsonl",
    ...
    test_path = pickle.load(
        open(
            "/nfs/turbo/justincj-turbo/tiangel/improvingVLM/packages/LLaVA-Plus/eval_data/two_img_dataset/ActivityDataset/sampled_frames.pkl",
            "rb",
        )
    )
    ...
    for i in range(len(pairs)):
        ...
        source_img = test_path[pairs[i][0]]["start_image_path"]
        target_img = test_path[pairs[i][0]]["end_image_path"]

        content = f"""
        Given two images sampled from a video clip, please design some questions and give their corresponding answers.
        The questions should require information and strong reasoning ability from both images to be answered, which is very important.
        Note these two images have clear temporal relationship, and the second image is the continuation of the first image.
        The questions should consider the temporal relationship between the two images.

        Avoid questions such as "What's the difference between the images?" or "How can we modify image A to resemble image B?" Accuracy in the answers is the highest priority
        Remember to keep the questions and answers high-quality and creative, while maintaining precise and concise.
        ...
        """
        # content = f"""
        # We are testing AI's ability to understand a pair of images.
        # Given two images, please propose several questions along with their correspondin answers.
        # The question should require information from both images to be answered and need comprehensive understanding and reasoning ability to answer.
        # Try to keep the questions and answers high-quality and creative, while maintaining precise and concise.
        # Avoid questions such as "What's the difference between the images?" or "How can we modify image A to resemble image B?"
        # Accuracy in the answers is the highest priority.

        # Keep the question and answers concise and precise.
