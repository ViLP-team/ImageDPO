...
from llava.train.llava_trainer import LLavaDPOTrainer, LLaVATrainer
...
class DPODataset(Dataset):
    def __init__(
        self, tokenizer: transformers.PreTrainedTokenizer, multimodal_cfg: dict
    ):
        super().__init__()

        self.tokenizer = tokenizer
        # Assume data path already exist
        self.list_data_dict = json.load(open(multimodal_cfg["data_path"]))
...
def encode_multimodal_preference_sample(source, tokenizer, multimodal_cfg):
    ...
    win_conv = copy.deepcopy(
        [source["question"], {"from": "gpt", "value": source["chosen"]}]
    )
    rej_conv = copy.deepcopy(
        [source["question"], {"from": "gpt", "value": source["rejected"]}]
    )
    if "image" in source:
        ...
        if isinstance(source["image"], str):
            ...
            image = Image.open(os.path.join(image_folder, source["image"])).convert(
                "RGB"
            )
...
class DataCollatorForDPODataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # NOTE: Ang: We seem don't have these `logp`. Not sure as I don't have gpu to run it.
