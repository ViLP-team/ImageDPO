...
from trl.trainer import DPOTrainer, Image_DPOTrainer
...
class UniformLengthGroupedSampler(torch.utils.data.Sampler):
    """Sampler that allows uniform sampling across concatenated datasets and
    can group samples by similar lengths if required."""
    ...
    def __init__(
        ...
        self.lengths = (
            lengths if lengths is not None else [1] * len(dataset)
        )  # Default to no grouping

        if group_by_modality:
            self.indices = self.group_by_length()
        else:
            self.indices = np.random.permutation(len(self.dataset))

    def group_by_length(self):
        # Group indices by similar lengths
        indices_and_lengths = list(enumerate(self.lengths))
        indices_and_lengths.sort(key=lambda x: x[1])
        sorted_indices = [x[0] for x in indices_and_lengths]
        return np.array(sorted_indices)

    def __iter__(self):
        # If not grouping by modality, shuffle each time to ensure randomness
        if not self.group_by_modality:
            self.indices = np.random.permutation(self.indices)
        # Return indices for this worker
        total_size = self.batch_size * self.world_size
        num_samples = len(self.indices) // total_size * total_size
        return iter(self.indices[:num_samples].tolist())
...
class LLaVATrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_modality_length:
            ...
            # )
            return UniformLengthGroupedSampler(
...
class LLavaDPOTrainer(LLaVATrainer):
    ...
    def __init__(self, **kwargs):
        ...
        # NOTE: Angs' change (might be wrong).
        # We need to have a ref model here (which might be an EMA).
        # https://github.com/huggingface/trl/blob/e90e8d91d2265e484f229c45a5eb8982f94a2936/trl/trainer/dpo_trainer.py#L295 initializes the ref model
        # it requires a special wrapper.
        # Let's try a very naive way first
        self.create_ref_model()
        # Let's have a iteration counter to keep track of the number of iterations, so that we can update the ref model.
    ...
    def compute_loss(self, model: Module, inputs: dict, return_outputs=False):
        ...
        # NOTE: we cannot get these logp from data dict.
        ...
        # NOTE: Ang's change(it might be wrong). We need to calculate the ref_win_logp and ref_rej_logp
        with torch.no_grad(), nullcontext():
            with self.null_ref_context():
                ref_concatenated_logp = forward_DPO(
                    self.model,
                    concatenated_input_ids,
                    concatenated_labels,
                    concatenated_attention_mask,
        ...
        if self.args.dpo_token_weighted:
            ...
            # # NOTE: Ang's change(it might be wrong). In this case, the `concatenated_logp` is `token_log_prob`. We need to split it into `ref_win` and `ref_rej`
            ref_win_per_token_logp, ref_rej_per_token_logp = (
                ref_concatenated_logp.split([win_size, rej_size])
            )
            ...
            ref_concatenated_logp = compute_weighted_logp(
                ref_concatenated_logp,
                concatenated_labels,
                concatenated_token_weight,
                self.args.dpo_use_average,
            )
        ...
        # NOTE: Ang's change (it could be wrong). I feel the `ref_win_logp` and `ref_ref_logp` are the same as ``
        policy_win_logp, policy_rej_logp = concatenated_logp.split([win_size, rej_size])
        ref_win_logp, ref_rej_logp = ref_concatenated_logp.split([win_size, rej_size])
        ...
        metrics[f"rewards_{train_test}/chosen"] = (
            self._nested_gather(chosen_rewards.mean()).mean().item()
        )
        metrics[f"rewards_{train_test}/rejected"] = (
            self._nested_gather(rejected_rewards.mean()).mean().item()
        )
        metrics[f"rewards_{train_test}/accuracies"] = (
            self._nested_gather(reward_accuracies.mean()).mean().item()
        )
        ...
        metrics[f"logps_{train_test}/rejected"] = (
            self._nested_gather(policy_rej_logp.mean()).mean().item()
        )
        metrics[f"logps_{train_test}/chosen"] = (
            self._nested_gather(policy_win_logp.mean()).mean().item()
        )
        metrics[f"logps_{train_test}/ref_rejected"] = (
            self._nested_gather(ref_rej_logp.mean()).mean().item()
        )
        metrics[f"logps_{train_test}/ref_chosen"] = (
            self._nested_gather(ref_win_logp.mean()).mean().item()
        )
        metrics[f"loss_{train_test}"] = loss.item()
        # metrics[f'batch_size'] = len(win_labels)
        self.log(metrics)

        # TODO: Updating ref model

        return loss


class LLaVADPOTrainer_dpotrainer(DPOTrainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
...
class LLaVADPOTrainer_dpotrainer_img(Image_DPOTrainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
