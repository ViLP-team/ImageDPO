...
from llava.train.llava_trainer import LLaVADPOTrainer_dpotrainer_img

import sys
sys.path.insert(0,'/nfs/turbo/justincj-turbo/tiangel/improvingVLM/packages/LLaVA-Plus/llava/train/trl')
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "good_image" in sources[0]:
            image_file = self.list_data_dict[i]["good_image"]
            ...
            if self.data_args.image_aspect_ratio == "pad":
                ...
                good_image = processor.preprocess(image, return_tensors="pt")[
                    "pixel_values"
                ][0]
            else:
                good_image = processor.preprocess(image, return_tensors="pt")[
            ...
            image_file = self.list_data_dict[i]["bad_image"]
            image_folder = self.data_args.image_folder
            processor = self.data_args.image_processor
            image = Image.open(os.path.join(image_folder, image_file)).convert("RGB")
            if self.data_args.image_aspect_ratio == "pad":
                ...
                bad_image = processor.preprocess(image, return_tensors="pt")[
                    "pixel_values"
                ][0]
            else:
                bad_image = processor.preprocess(image, return_tensors="pt")[
        ...
        if "image" in self.list_data_dict[i]:
            data_dict["good_image"] = good_image
            data_dict["bad_image"] = bad_image
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict["good_image"] = torch.zeros(
                3, crop_size["height"], crop_size["width"]
            )
            data_dict["bad_image"] = torch.zeros(
                3, crop_size["height"], crop_size["width"]
            )
        return data_dict
...
class DataCollatorForSupervisedDataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "good_image" in instances[0]:
            images = [instance["good_image"] for instance in instances]
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch["good_image"] = torch.stack(images)
            else:
                batch["good_image"] = images
        
        if "bad_image" in instances[0]:
            images = [instance["bad_image"] for instance in instances]
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch["bad_image"] = torch.stack(images)
            else:
                batch["bad_image"] = images
        return batch
...
class DPODataset(Dataset):
    ...
    # try:
        ...
        image_file = data_dict['good_image']
        image_folder = self.data_args.image_folder
        processor = self.data_args.image_processor
        image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
        if self.data_args.image_aspect_ratio == 'pad':
            ...
            good_image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        else:
            good_image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]


        image_file = data_dict['bad_image']
        image_folder = self.data_args.image_folder
        processor = self.data_args.image_processor
        image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
        if self.data_args.image_aspect_ratio == 'pad':
            def expand2square(pil_img, background_color):
                ...
                if width == height:
                    ...
                    return result
            image = expand2square(image, tuple(int(x*255) for x in processor.image_mean))
            bad_image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        else:
            bad_image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]


        data_dict['prompt'] = data_dict['question']['value']
        data_dict['good_image'] = good_image
        data_dict['bad_image'] = bad_image
...
class DPODataCollator(DPODataCollatorWithPadding):
    tokenizer: transformers.PreTrainedTokenizer = transformers.GPT2Tokenizer.from_pretrained("openai-community/gpt2")
    pad_token_id: int = 0
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        tokenized_batch = []
        Xs, keys = [], []
        good_image_collect, bad_image_collect = [], []
        for feature in features:
            prompt = feature["prompt"]
            chosen = feature["chosen"]
            rejected = feature["rejected"]
            has_image = feature['has_image']
            good_image_collect.append(feature['good_image'])
            bad_image_collect.append(feature['bad_image'])
        ...
        padded_batch['good_image'] = good_image_collect  # we do not change the key's name.
        padded_batch['bad_image'] = bad_image_collect
        return padded_batch
...
def train():
    ...
    #hf_dataset = hf_Dataset.from_list(data_list)
    ...
    trainer = LLaVADPOTrainer_dpotrainer_img(
        model,
        args=training_args,
