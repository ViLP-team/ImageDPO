...
### the below is my added function
...
class CustomConcatDataset(ConcatDataset):
    ...
    def modality_lengths(self):
        # Aggregate modality lengths from all constituent datasets
        total_lengths = []
        for dataset in self.datasets:
            if hasattr(dataset, 'modality_lengths'):
                total_lengths.extend(dataset.modality_lengths)
            else:
                # Handle case where dataset does not have modality_lengths
                # You might want to compute it differently or raise an error
                raise ValueError("Dataset does not have a 'modality_lengths' property")
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # alternate_batches: bool = field(default=False, metadata={"help": "Whether to alternate batches between datasets."})
...
class LazySupervisedDataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            length_list.append(sum(len(conv['value'].split()) for conv in sample['qa']) + img_tokens)
        return length_list
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = sum(len(conv['value'].split()) for conv in sample['qa'])
        ...
        while True:
            ...
            if 'image' in sources[0]:
                ...
                sources = preprocess_multimodal(
                    ...
                    copy.deepcopy([e["qa"] for e in sources]),
                    self.data_args)
                #self.data_args: DataArguments(data_path='./llava_v1_5_mix665k.json', lazy_preprocess=True, is_multimodal=True, image_folder='./playground/data', image_aspect_ratio='pad')
            elif 'images' in sources[0]:
                image_file_list = self.list_data_dict[i]['images']
                image_folder = self.data_args.image_folder
                processor = self.data_args.image_processor
                image_list = []
                ...
                sources = copy.deepcopy([e["qa"] for e in sources])
            # this preprocess will produce warning if sources exceed the max length
            ...
            # if the input_ids is too long (currently, 2048), we will skip it
            # one image_token will be added to the input_ids, which is [576, 512]
            if (('image' in self.list_data_dict[i]) and (data_dict['input_ids'].shape[1] <= self.tokenizer.model_max_length - 35 - 575)) or (('images' in self.list_data_dict[i]) and (data_dict['input_ids'].shape[1] <= self.tokenizer.model_max_length - 35 - 575*len(image_file_list) - (len(image_file_list)-1))):
                break
            else:
                i = torch.randint(0, len(self.list_data_dict), (1,)).item()
        if isinstance(i, int):
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            cur_images = []
            if type(instance['images']) is list:
                for i in range(len(instance['images'])):
                    cur_images.append(instance['images'][i].unsqueeze(0))
                images.append(cur_images)
            else:
                images.append(instance['images'].unsqueeze(0))
        batch['images'] = images

        #if 'images' in instances[0]:
        #    images = [instance['images'] for instance in instances]
        #    type_list = [type(x) for x in images]
        #    if list in type_list:
        #        final_img_list = []
        #        for instance in instances:
        #            if type(instance['images']) is list:
        #                final_img_list.append(torch.stack(instance['images']).unsqueeze(0))
        #            else:
        #                final_img_list.append(instance['images'])

        #    if type(instances[0]['images']) is list:
        #        final_img_list = []
        #        for instance in instances:
        #            final_img_list.append(torch.stack(instance['images']).unsqueeze(0))
        #        batch['images'] = torch.cat(final_img_list)
        #    else:
        #        try:
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = CustomConcatDataset(data_set_list)
...
def train():
    ...
    #instances=[train_dataset[0],train_dataset[-1],train_dataset[10000]]
    ...
    #        input_ids,
    #        batch_first=True,
    #        padding_value=data_module['data_collator'].tokenizer.pad_token_id)
    #labels = torch.nn.utils.rnn.pad_sequence(labels,
    #                                             batch_first=True,
    #                                             padding_value=IGNORE_INDEX)
    #input_ids = input_ids[:, :data_module['data_collator'].tokenizer.model_max_length]
    #labels = labels[:, :data_module['data_collator'].tokenizer.model_max_length]
    #batch = dict(
    #    input_ids=input_ids,
    #    labels=labels,
    #    attention_mask=input_ids.ne(data_module['data_collator'].tokenizer.pad_token_id),
    #)
