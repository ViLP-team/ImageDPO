# code from https://github.com/YiyangZhou/CSR/blob/main/train_csr/dpo_trainer.py
...
class DPOTrainer(Trainer):
    ...
    def _get_noisy_batch_logps(
        ...
        per_token_logps = torch.gather(
            new_chosen_logits.log_softmax(-1), dim=2, index=max_indices.unsqueeze(2)
        ).squeeze(2)
        ...
        rejected_model_kwargs = (
            {
                ...
                "decoder_input_ids": concatenated_batch.pop(
                    "rejected_decoder_input_ids", None
                ),
            }
            if self.is_encoder_decoder
            else {}
